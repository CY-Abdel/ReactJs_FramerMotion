...
const Contact = () => {
  ...
  const sendEmail = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Vérifier si formRef.current n'est pas null
    if (formRef.current) {
      emailjs.sendForm('service_z5hf9xb', 'template_ku1624b', formRef.current, 'ouWyy3guNIdXS4ZjY')
        .then((result) => {
          setSuccess(true);
        })
        .catch((error) => {
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        <motion.h1 variants={variants}>Transformons vos idées  en réalité</motion.h1>

        <motion.div className="item" variants={variants}>
          <h2>Email</h2>
          <span>juba_vde@react-motion.fr</span>
        </motion.div>

        <motion.div className="item" variants={variants}>
          <h2>Adresse</h2>
          <span>Victor Hugo Paris </span>
        </motion.div>
